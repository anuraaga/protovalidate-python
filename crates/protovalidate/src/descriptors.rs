...
/// Decodes a serialized `FileDescriptorSet`.
pub(crate) fn decode_file_set(bytes: &[u8]) -> Result<FileDescriptorSet, String> {
    // Descriptor sets are trusted schema, and a large one exceeds the
    // element-memory bound buffa applies to untrusted input.
    buffa::DecodeOptions::new()
        .with_element_memory_limit(usize::MAX)
        .decode_from_slice(bytes)
        .map_err(|error| format!("could not parse FileDescriptorSet: {error}"))
}

/// Decodes a serialized `FileDescriptorProto`.
pub(crate) fn decode_file(bytes: &[u8]) -> Result<FileDescriptorProto, String> {
    buffa::DecodeOptions::new()
        .with_element_memory_limit(usize::MAX)
        .decode_from_slice(bytes)
        .map_err(|error| format!("could not parse FileDescriptorProto: {error}"))
}

/// A pool seeded with the `buf.validate` schema.
pub(crate) fn base_pool() -> DescriptorPool {
    let set = decode_file_set(BASE_DESCRIPTOR_SET).expect("embedded descriptor set decodes");
    DescriptorPool::new(set).expect("embedded descriptor set links")
}

/// The `FieldDescriptorProto.Type` of a field, as protovalidate reports it
/// in field paths: `TYPE_GROUP` for delimited-encoded messages.
...
/// The name protobuf prints for a type, as in `field type does not match
...
fn path_element_named(field: &FieldDescriptor, name: String) -> FieldPathElement {
    FieldPathElement {
        field_number: Some(i32::try_from(field.number()).unwrap_or(i32::MAX)),
...
/// The fields of well-known types the rules read through a runtime, as the
/// walker asks for them.
pub(crate) mod wkt {
    ...
    pub(crate) static ANY_TYPE_URL: LazyLock<Field> = LazyLock::new(|| {
        Field::well_known(
            1,
            "type_url",
    ...
    /// `seconds` of `google.protobuf.Duration` and `Timestamp`, which
    /// declare it alike.
    pub(crate) static SECONDS: LazyLock<Field> = LazyLock::new(|| {
        Field::well_known(
            1,
            "seconds",
    ...
    pub(crate) static NANOS: LazyLock<Field> = LazyLock::new(|| {
        Field::well_known(2, "nanos", Kind::Singular(Singular::Scalar(Scalar::Int32)))
    });
    /// `google.protobuf.FieldMask.paths`.
    pub(crate) static FIELD_MASK_PATHS: LazyLock<Field> = LazyLock::new(|| {
        Field::well_known(1, "paths", Kind::List(Singular::Scalar(Scalar::String)))
    });
}

/// The pieces of the `buf.validate` schema referred to by position: the path
/// elements of the structural rules and the container rules (`required`,
/// `cel`, `repeated.items`, `map.keys`, ...), and the `FieldRules` message
/// every standard rule path starts from.
pub(crate) struct Schema {
    ...
    pub r#enum: FieldPathElement,
    pub enum_defined_only: FieldPathElement,
    pub any: FieldPathElement,
    pub any_in: FieldPathElement,
    pub any_not_in: FieldPathElement,
}
...
impl Schema {
    pub(crate) fn new(pool: &DescriptorPool) -> Self {
        ...
        Self {
            ...
            r#enum: element(FIELD_RULES, "enum"),
            enum_defined_only: element("buf.validate.EnumRules", "defined_only"),
            any: element(FIELD_RULES, "any"),
            any_in: element("buf.validate.AnyRules", "in"),
            any_not_in: element("buf.validate.AnyRules", "not_in"),
        }
    }
...
/// The descriptors a validator reflects over: the pool and the schema
/// positions within it.

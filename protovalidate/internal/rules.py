        ...
        for runner in self._cel:
            ...
            if result_type == cel.Type.BOOL:
                if not result.plain_value():
                    message = runner.rule.message
                    if len(message) == 0:
                        message = f'"{runner.rule.expression}" returned false'
                    ctx.add(
                        Violation(
        ...
        if type_case is not None:
            ...
            for list_field, _ in rules.ListFields():
                if validate_pb2.predefined in list_field.GetOptions().Extensions:  # ty: ignore[unsupported-operator]
                    for cel_rule in list_field.GetOptions().Extensions[validate_pb2.predefined].cel:  # ty: ignore[invalid-argument-type]
                        self.add_rule(
                            env,
                            cel_rule,
